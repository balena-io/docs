...
async function downloadFolderFromGithub(repo, folderPath, ref, targetDir) {
	...
	for (const file of files) {
		...
		// This is the folder that the given file is in, i.e. `pages/foo/bar`
		const dirName = path.dirname(dest);
		// This is just the path that should be shown in SUMMARY.md, like `foo/bar/fizz.md`
		...
		try {
			...
			// i.e. `foo/bar/README.md`
			const readmePath = path.join(dirName, 'README.md');
			// Count the slashes in the path to determine how many times to indent
			// No slashes (root level) means no indent, one slash (inside a folder) means one indent, two slashes (folder inside a folder) means two indents, etc.
			const indentDepth = destLatter.split('/').length;
			// We create a README for the targetDir outside of this function if all goes well as we need the pageTitle for the versioned folders, which differs from the versioned folder names
			// If the current dir is not the targetDir, and there is no README, we create one with a title based on the folder name
			if (dirName !== targetDir && !fs.existsSync(readmePath)) {
				...
				// Create the README summary line, indenting based on folder depth
				// We subtract 1 from the indent depth because the README represents the folder itself, so it should be indented one level less than the files within it
			...
			// Create the file summary line, indenting based on folder depth
	...
	for (const source of versionedSources) {
		...
		for (const version of versionsToTrack) {
			...
			if (file.endsWith('/')) {
				...
				console.log(
					"Trying fallback file since folder download didn't work...",
				);
