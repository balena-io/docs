...
A nice project to try is the \[balena-\{{ $language.id \}}-hello-world]\(https://github.com/balena-io-examples/balena-\{{ $language.id \}}-hello-world) project. It's a \{{ $language.name \}} web server that serves a static page on port 80. To get started, \[download the project]\(https://github.com/balena-io-examples/balena-\{{ $language.id \}}-hello-world/archive/master.zip) as a zipped file from GitHub, unzip it and open a terminal in the root of the extracted project directory.
