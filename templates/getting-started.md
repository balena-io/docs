...
Enter a fleet name, select the **{{ $device.name }}** device type, choose the *Starter* [fleet type](/learn/accounts/fleet-types), and click **Create new fleet**:
...
A nice project to try is the [balena-{{ $language.id }}-hello-world](https://github.com/balena-io-examples/balena-{{ $language.id }}-hello-world) project.
It's a {{ $language.name }} web server that serves a static page on port 80. To get started, [download the project](https://github.com/balena-io-examples/balena-{{ $language.id }}-hello-world/archive/master.zip)
