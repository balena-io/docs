...
Enter a fleet name, select the **{{ $device.name }}** device type, and click **Create new fleet**:
...
This will download a zipped image file with a name like `balena-First-Fleet-{{ $device.id }}-6.10.24-v17.4.2.img.zip`. The contents of this image file are the same as what Etcher would have received if you had chosen the **Flash**. The difference is that you could reuse this image file repeatedly without having to fill out the form again.
