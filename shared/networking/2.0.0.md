...
Under the hood resin WiFi connect is interacting with NetworkManager via its [DBUS][dbus-link] API. The DBUS API is a useful way to interact with the resinOS host NetworkManager and there are DBUS bindings for most languages. Below is minimal python example from the [NetworkManager examples][network-manager-examples], which creates a new NetworkManager connection file, that can be used to enable connecting to a new Wifi access point.
...
For other network setting needs, such as modifying existing network connection files, or listing connection information, please see the [Network Manager examples][network-manager-examples].
