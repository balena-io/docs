...
Finally, please note that while we are working on adding IPv6 support for all balena-cloud.com services, some services, like our registry (`registry2.balena-cloud.com`) still are IPv4-only so devices on IPv6-only networks will most likely experience issues downloading updates. Please make sure devices are in an IPv4 capable network if you observe this problem.
