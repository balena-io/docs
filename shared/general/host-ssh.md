...
* `balena.service` - The [{{ $names.engine.lower }}][balena-engine] service, the modified Docker daemon fork that allows the management and running of fleet service images, containers, volumes, and networking.
* `balena-supervisor.service` - The {{ $names.company.short }} Supervisor service, responsible for the management of fleets, including downloading updates for and self-healing (via monitoring) of those fleets, variables (fleet/device), and exposure of these services to fleets via an endpoint.
* `dbus.service` - The DBus daemon socket which can be used by fleets by applying the _io.balena.features.dbus_ [label][labels], which exposes it in-container. This allows fleets to control several host OS features, including the Network Manager.
