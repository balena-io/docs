...
* Docker socket exposed on via port `2377`, which allows `{{ $names.company.lower }} local push` to do remote Docker builds on the target device.
...
Below is a representative example from the Raspberry Pi family, which helps explain [meta-{{ $names.company.lower }}-raspberrypi/conf/samples/bblayers.conf.sample]({{ $links.githubOS }}/resin-raspberrypi/blob/master/layers/meta-resin-raspberrypi/conf/samples/bblayers.conf.sample).

| Layer Name                        | Repository                                                                      | Description                                                                           |
|-----------------------------------|-------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| meta-{{ $names.company.lower }}                        | {{ $links.githubOS }}/meta-resin                                        | This repository enables building {{ $names.os.lower }} for various devices                         |
| meta-{{ $names.company.lower }}-jethro                 | {{ $links.githubOS }}/meta-resin                                        | This layer enables building {{ $names.os.lower }} for jethro supported BSPs                        |
