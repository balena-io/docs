...
2. Advanced HUP: There are certain upgrade paths where Standard HUP is not compatible. This update mechanism is usually more involved and does not provide features such as automatic rollback and delta updates. Advanced HUP is usually used for certain specific scenarios and subsequent updates can then be done via Standard HUP. For e.g. The partition layout for NVIDIA JetPack 4-based {{ $names.company.lower }}OS differs from JetPack 5-based {{ $names.company.lower }}OS. If a device is running already running JetPack-4 {{ $names.company.lower }}OS, you can use Advanced HUP to update to a JetPack 5-based {{ $names.company.lower }}OS. Subsequent updates can be carried out using Standard HUP.
...
<TODO: should we mention Stand HUP file naming convention?>
...
1. Partition layout changes from one OS version to another. E.g:
    
    -  The partition layout for NVIDIA JetPack 4-based {{ $names.company.lower }}OS differs from JetPack 5-based {{ $names.company.lower }}OS. If a device is running already running JetPack-4 {{ $names.company.lower }}OS, you can use Advanced HUP to update to a JetPack 5-based {{ $names.company.lower }}OS
    - {{ $names.company.upper }} releases a new version of an OS for a certain device type whereby the root partition has been increased to accomodate more kernel module and drivers
...
2. Changing device type. E.g: Switching from a the 32-bit OS variant running on Raspberry Pi 3 to a 64-bit variant of {{ $names.company.lower }}OS

__Note:__ If you've used Advanced HUP, it does not mean necessarily updates won't be compatible with Standard HUP. E.g If a device running 32-bit  {{ $names.company.lower }}OS on a Raspberry Pi 3 has been updated to a 64-bit variant of {{ $names.company.lower }}OS, Standard HUP can be used for future updates to the 64-bit OS. In this case, Advanced HUP is only used once to migrate the device from one OS architecture to another.

### Caveats
When using the Advanced HUP, it’s important to be aware of several limitations compared to the Standard HUP mechanism.

#### Data loss
Data loss may occur, as the process can involve overwriting partitions and filesystems. This means that any data not safeguarded through proper backups or persistent storage strategies could be irretrievably lost.
...
#### No delta updates
The OS image is downloaded via the api.
...
- copies required files and artefacts to RAMFS
- Unmounts partitions
- Flashes the target OS image to disk already running {{ $names.company.lower }}OS
...
__Note:__ Because the disk running {{ $names.company.lower }}OS is overwritten with the target OS image, there is no automated rollback mechanism when using Advanced HUP. However, if the process is interrupted before the disk is overwritten, the device is rebooted and it will boot into the OS that was already running.
