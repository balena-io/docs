...
9. Once provisioning is complete, the board will perform one of the following actions: - restart and boot balenaOS automatically, if \[installer.migrate.force]\[installer.migrate] has been set in \[config.json]\[config\_json]. - shut down if \[installer.migrate.force]\[installer.migrate] has not been set in \[config.json]\[config\_json]. Unplug the flasher USB flash drive before powering the AGX Orin Devkit back on.
