...
**Note:** Apps are an experimental feature. To enable them, go to _Preferences → Experimental Options_ and make sure "Display Apps" is checked.
...
Apps exist today for those developers who wish to share what they have built on \[balenaHub]\[balenaHubUrl] in order to enable deployment to other fleets owned by other people using balenaCloud. Apps are created within balenaCloud, releases are pushed in the exact same way as with fleets, and then they can be [marked public](apps.md#publishing-an-app-on-balenahub) to publish them on balenaHub and share with others. In this manner, one App can be installed on multiple fleets.
