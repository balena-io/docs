...
For fleets with [multiple containers][multicontainer], a `docker-compose.yml` file will need to be included at the root of your project. This configuration file specifies the services that make up your fleet, as well as the system resources each service has access to. Fleets with a single container will have a default `docker-compose.yml` file generated if none is included.
