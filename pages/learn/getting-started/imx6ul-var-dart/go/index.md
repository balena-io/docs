# Getting Started: Variscite DART-6UL and Go

In this guide, we will help you get started with balenaCloud by:

- Setting up your **Variscite DART-6UL** device and bringing it online on the balenaCloud dashboard.
- Deploying a **Go** hello-world project on the device.
...
<img style="float: right;padding-left: 10px;" src="/img/imx6ul-var-dart/imx6ul-var-dart.webp">

* A [Variscite DART-6UL](https://www.variscite.com/products/system-on-module-som/cortex-a7/dart-6ul-freescale-imx-6ul), along with the baseboard that allows flashing from an SD card.
* A 4GB or larger microSD card. The [speed class](https://en.wikipedia.org/wiki/Secure_Digital#Speed_class_rating) of the card also matters - a class 10 card or above is the way to go.
* An ethernet cable.
...
Enter a fleet name, select the **Variscite DART-6UL** device type, choose the *Starter* [fleet type](/learn/accounts/fleet-types), and click **Create new fleet**:

<img src="/img/getting-started/create-fleet.webp" width="80%">

You'll then be redirected to the summary of the newly created fleet, where you can add your first Variscite DART-6UL.
...
balenaCloud builds a custom balenaOS image configured for Variscite DART-6UL which allows the device to provision and join the new fleet you created automatically. Start by clicking **Add device** on the fleet summary. Your device type will be preselected here since you already chose it when creating the fleet. Other device types of the same [architecture](/reference/hardware/devices/) can also be picked to join the fleet.
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-imx6ul-var-dart-2.80.3+rev1-v12.7.0.img.zip`.
...
Insert the SD card to the host machine.,Write the balenaOS file you downloaded to the SD card. We recommend using &lt;a href&#x3D;&quot;https://etcher.balena.io/&quot;&gt;Etcher&lt;/a&gt;.,Wait for writing of balenaOS to complete.,Remove the SD card from the host machine.,Insert the freshly flashed SD card into the Variscite DART-6UL.,&lt;strong role&#x3D;&quot;alert&quot;&gt;Warning!&lt;/strong&gt; This will also completely erase internal storage medium, so please make a backup first.,Set the SW1 and SW2 boot switches to 00 - SD boot Connect power to the Variscite DART-6UL.,Wait for the Variscite DART-6UL to finish flashing and shutdown. Please wait until the green LED stops flashing.,Remove the SD card from the Variscite DART-6UL.,Set the SW1 and SW2 boot switches to 01 - eMMC boot,Remove and re-connect power to the Variscite DART-6UL to boot the device.

When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Variscite DART-6UL](/faq/troubleshooting/imx6ul-var-dart) or try our [support channels](/support/).
...
98264 First-Fleet  balena CLI/first-fleet    Variscite DART-6UL    0              0
```

A nice project to try is the [balena-go-hello-world](https://github.com/balena-io-examples/balena-go-hello-world) project.
It's a Go web server that serves a static page on port 80. To get started, [download the project](https://github.com/balena-io-examples/balena-go-hello-world/archive/master.zip)
...
After the download, you should now have a Go web server running on your device and see some logs on your dashboard. 
