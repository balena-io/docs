# Getting Started: BeaglePlay and Go

In this guide, we will help you get started with balenaCloud by:

- Setting up your **BeaglePlay** device and bringing it online on the balenaCloud dashboard.
...
* Your BeaglePlay device you want to get started with. Check out all of our [supported devices](/reference/hardware/devices/).
...
Enter a fleet name, select the **BeaglePlay** device type, choose the *Starter* [fleet type](/learn/accounts/fleet-types), and click **Create new fleet**:

<img src="/img/getting-started/create-fleet.webp" width="80%">

You'll then be redirected to the summary of the newly created fleet, where you can add your first BeaglePlay.
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-beagleplay-2.80.3+rev1-v12.7.0.img.zip`.
...
Insert the SD card to the host machine.,Write the balenaOS file you downloaded to the SD card. We recommend using &lt;a href&#x3D;&quot;https://etcher.balena.io/&quot;&gt;Etcher&lt;/a&gt;.,Wait for writing of balenaOS to complete.,Remove the SD card from the host machine.,Insert the freshly flashed SD card into the BeaglePlay.,&lt;strong role&#x3D;&quot;alert&quot;&gt;Warning!&lt;/strong&gt; This will also completely erase internal storage medium, so please make a backup first.,Press and hold the USR and RST buttons, then apply power to the device. Once power is applied, release both USR and RST buttons.,Wait for the BeaglePlay to finish flashing and shutdown. Please wait until all LEDs are off.,Remove the SD card from the BeaglePlay.,Re-connect power to the BeaglePlay to boot the device.

When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for BeaglePlay](/faq/troubleshooting/beagleplay) or try our [support channels](/support/).
...
98264 First-Fleet  balena CLI/first-fleet    BeaglePlay    0              0
```
