...
Enter a fleet name, select the **Coral Dev Board** device type, choose the _Starter_ [fleet type](../accounts/fleet-types.md), and click **Create new fleet**:
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-coral-dev-6.10.24-v17.4.2.img.zip`.
...
<img src="../../.gitbook/assets/insert-sd.gif" alt="insert SD card">
  ...
  * Set the BOOT_SELECT switch to the SD-CARD position Connect power to the Coral Dev Board
  ...
  * Set the BOOT_SELECT switch to the eMMC position,Remove and re-connect power to the Coral Dev Board to boot the device.

When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Coral Dev Board](/faq/troubleshooting/coral-dev) or try our [support channels](../accounts/support-access.md).
