...
Enter a fleet name, select the **Nvidia Jetson TX2 NX (with Xavier NX Devkit)** device type, choose the _Starter_ [fleet type](../../learn/accounts/fleet-types.md), and click **Create new fleet**:
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-jetson-tx2-nx-devkit-2.80.3+rev1-v12.7.0.img.zip`.
...
When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Nvidia Jetson TX2 NX (with Xavier NX Devkit)](/faq/troubleshooting/jetson-tx2-nx-devkit) or try our [support channels](../../learn/accounts/support-access.md).
