# Getting Started with CTI Photon TX2 NX
...
* Your CTI Photon TX2 NX device you want to get started with. Check out all of our [supported devices](../../reference/hardware/devices.md).
...
Enter a fleet name, select the **CTI Photon TX2 NX** device type, choose the _Starter_ [fleet type](../accounts/fleet-types.md), and click **Create new fleet**:
...
balenaCloud builds a custom balenaOS image configured for CTI Photon TX2 NX which allows the device to provision and join the new fleet you created automatically. Start by clicking **Add device** on the fleet summary. Your device type will be preselected here since you already chose it when creating the fleet. Other device types of the same [architecture](../../reference/hardware/devices.md) can also be picked to join the fleet.
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-photon-tx2-nx-6.10.24-v17.4.2.img.zip`.
...
When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for CTI Photon TX2 NX](/faq/troubleshooting/photon-tx2-nx) or try our [support channels](../accounts/support-access.md).
