# Getting Started: Nvidia Jetson TX2 NX (with Xavier NX Devkit) and Go
...
Enter a fleet name, select the **Nvidia Jetson TX2 NX (with Xavier NX Devkit)** device type, choose the *Starter* [fleet type](/learn/accounts/fleet-types), and click **Create new fleet**:
...
When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Nvidia Jetson TX2 NX (with Xavier NX Devkit)](/faq/troubleshooting/jetson-tx2-nx-devkit) or try our [support channels](/support/).
