...
Enter a fleet name, select the **Variscite VAR-SOM-MX7** device type, choose the _Starter_ [fleet type](../accounts/fleet-types.md), and click **Create new fleet**:
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-imx7-var-som-6.10.24-v17.4.2.img.zip`.
...
* Insert the freshly flashed SD card into the Variscite VAR-SOM-MX7.
* 
...
* Set the SW1 BOOT-SELECT switch to SD-CARD. Connect power to the Variscite VAR-SOM-MX7.
* Wait for the Variscite VAR-SOM-MX7 to finish flashing and shutdown. Please wait until all LEDs are off.
* Remove the SD card from the Variscite VAR-SOM-MX7.
* Set the SW1 BOOT-SELECT switch to eMMC.,Remove and re-connect power to the Variscite VAR-SOM-MX7. to boot the device.

When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Variscite VAR-SOM-MX7](/faq/troubleshooting/imx7-var-som) or try our [support channels](../accounts/support-access.md).
