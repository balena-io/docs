...
Enter a fleet name, select the **Variscite DART-6UL** device type, choose the _Starter_ [fleet type](../accounts/fleet-types.md), and click **Create new fleet**:
...
Finally, click the **Download balenaOS** button. When the download completes, you should have a zipped image file with a name like `balena-First-Fleet-imx6ul-var-dart-6.10.24-v17.4.2.img.zip`.
...
* Insert the freshly flashed SD card into the Variscite DART-6UL.
* 
...
* Set the SW1 and SW2 boot switches to 00 - SD boot Connect power to the Variscite DART-6UL.
* Wait for the Variscite DART-6UL to finish flashing and shutdown. Please wait until the green LED stops flashing.
* Remove the SD card from the Variscite DART-6UL.
* Set the SW1 and SW2 boot switches to 01 - eMMC boot,Remove and re-connect power to the Variscite DART-6UL to boot the device.

When complete, after a minute or two the device should appear on your balenaCloud [dashboard](https://dashboard.balena-cloud.com/), and you should now be ready to deploy some code. If you are not able get the device to appear on the dashboard, then check out our [troubleshooting guide for Variscite DART-6UL](/faq/troubleshooting/imx6ul-var-dart) or try our [support channels](../accounts/support-access.md).
