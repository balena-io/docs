...
**Note:** When filtering by \[device tags]\[tags], you can filter by name, value, or both.
...
**Note:** A device cannot have more than one tag with the same name, even if the values are different. If you create a new tag with the same name as an existing tag, the existing tag will be replaced.
...
**Note:** Device tags can also be created and managed programmatically \[via the API]\[api-example].
