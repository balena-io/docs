...
| Status                    | Description                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Operational**           | The device is functioning normally and communicating with the cloud within the expected time interval. In the standard configuration, this means both [cloudlink](../welcome/security.md#cloudlink) and the device heartbeat API are working. If [cloudlink is disabled](../../reference/supervisor/bandwidth-reduction.md#side-effects-warnings), the device will still appear `Operational` if the heartbeat API is functional. |
...
| **Disconnected**          | The device is not connected to cloudlink and has not communicated via the Heartbeat API for a duration exceeding the [API polling interval](configuration.md#variable-list). This status indicates that the device is completely unable to send or receive any data from the balena cloud.                                                                                                                                        |
...
| **Post Provisioning**     | The device has been [provisioned](../welcome/primer.md#device-provisioning) but has not yet been booted from its internal storage (e.g. eMMC or NVMe).                                                                                                                                                                                                                                                                            |
| **Inactive**              | The device has been [deactivated](../accounts/billing.md#inactive-devices) or has been [preregistered](/broken/pages/tAdmS7VhfWwHTNcoKQDc#id-5.2-preregistering-a-device) but has not yet connected to the balenaCloud API.                                                                                                                                                                                                       |
| **Frozen**                | The device has been frozen because it's outside the organization's allowance, or is in a paid [fleet type](../accounts/fleet-types.md) on a free tier organization.                                                                                                                                                                                                                                                               |
...
<figure><img src="../../.gitbook/assets/device_status (1).webp" alt=""><figcaption></figcaption></figure>
...
* **Cloudlink available, but heartbeat not**: The device is unable to communicate with the balenaCloud API. A device with cloudlink only is not able to apply any new changes made such as deploying new releases, applying service configuration values, or switching to local mode. However, since cloudlink is available, the device is accessible via SSH or the web terminal. When a device exhibits this state, the first troubleshooting step is to [run a healthcheck](/broken/pages/ST3eAfYvu3wFnVbQa4io#device-health-checks) and pull [device diagnostics](/broken/pages/ST3eAfYvu3wFnVbQa4io#device-diagnostics). This should help determine the reason the supervisor is not communicating with the balenaCloud backend.
...
| **Downloaded**       | All container images in the target release have been downloaded. **Note:** If the device holds an update lock, it will stop in this state until the lock is released (depending on the [update strategy](../deploy/release-strategy/update-strategies.md)).            |
...
| **Rejected**         | The target release has required services that do not meet the [necessary requirements](../develop/multicontainer.md#container-requirements) to run.                                                                                                                    |
...
If you find your device to be displaying a status which is unclear even with the above notes, visit the [device debugging masterclass](/broken/pages/T79VY1kNylIMs4lrtHQC) for more information.
