...
**Note:** This feature is available to customers on [standard paid plans](https://www.balena.io/pricing) - Prototype Plan, Pilot Plan, Production Plan, or Enterprise Plan.
...
Configuring an Identity Provider (IdP) as a login method requires a one-time setup process within both balenaCloud and the IdP itself. Refer to our [IdP specific documentation](../../../learn/accounts/idp-setup/microsoft-entra-saml-setup/) for detailed instructions on required configurations.

* [Link a SAML Identity Provider](enterprise-sso.md#link-a-saml-identity-provider)
  * [Associate Organizations and Teams](enterprise-sso.md#associate-organizations-and-teams)
    * [(Optional) Configure a Default Team](enterprise-sso.md#optional-configure-a-default-team)
* [Authenticating as a SAML User](enterprise-sso.md#authenticating-as-a-saml-user)
* [Setting up a new SAML user](enterprise-sso.md#setting-up-a-new-saml-user)
* [Log in with a SAML account](enterprise-sso.md#log-in-with-a-saml-account)
* [FAQs](enterprise-sso.md#faqs)
  * [How do I set up an Identity Provider?](enterprise-sso.md#how-do-i-set-up-an-identity-provider)
  * [Can I use any identity provider with balena’s Enterprise SSO?](enterprise-sso.md#can-i-use-any-identity-provider-with-balenas-enterprise-sso)
  * [Can I enforce SAML on all users in my organization?](enterprise-sso.md#can-i-enforce-saml-on-all-users-in-my-organization)
  * [How can I use API keys if SAML users can't create them?](enterprise-sso.md#how-can-i-use-api-keys-if-saml-users-cant-create-them)
  * [How do I delete a SAML account?](enterprise-sso.md#how-do-i-delete-a-saml-account)
  * [How do I delete an Identity Provider in balenaCloud?](enterprise-sso.md#how-do-i-delete-an-identity-provider-in-balenacloud)
  * [Why do I get an error when enabling 2FA on my SAML account?](enterprise-sso.md#why-do-i-get-an-error-when-enabling-2fa-on-my-saml-account)

## Link a SAML Identity Provider

To enable Single Sign-On (SSO) for balenaCloud organizations, you must establish a connection with your external Identity Provider (IdP). BalenaCloud supports all SAML 2.0 Identity Providers, and we provide examples for [Microsoft Entra ID](../../../learn/accounts/idp-setup/microsoft-entra-saml-setup/) (formerly Azure AD) and [Google Workspace](../../../learn/accounts/idp-setup/google-workspace-saml-setup/). This process assumes that you have already configured a [SAML 2.0 IdP and possess an XML certificate](../../../learn/accounts/idp-setup/microsoft-entra-saml-setup/) ready for upload.
...
**Note:** Removing organizations after IdP creation will not revoke access for SAML users who have previously authenticated with this IdP. However, new authentications will no longer include the removed organization. An IdP will always require at least one organization to be associated.

You have successfully configured SAML 2.0 for your balenaCloud Enterprise SSO. Your team can now access balenaCloud securely and seamlessly through the configured Identity Provider. For instructions on how your team can log in, refer to [Authenticating as a SAML/SSO User](enterprise-sso.md#authenticating-as-a-samlsso-user). If you encounter any issues or need further assistance, please contact our support team.
...
**Note**: If you unlink the default team in the Identity Provider configuration, it will not remove team access for SAML users who have already authenticated. However, new SAML users will no longer be automatically added to this team.
...
To delete a SAML account, you need to use the [balena SDK](https://docs.balena.io/reference/sdk/node-sdk/). This step is only required if you intend to [delete your IdP](enterprise-sso.md#how-do-i-delete-an-identity-provider-in-balenacloud). **Removing the user from your IdP will block their access to balenaCloud**, but their current session will remain active for up to 12 hours after their last login.

**Warning:** Ensure that there is at least one non-SAML admin user in your organization before deleting all SAML users in the Identity Providers (IdPs). Failure to do so may result in being locked out of your organization.
