...
excerpt: >-
  The Deploy with {{ $names.company.lower }} button allows users to do a
...
The above example uses a SVG logo as this renders better on GitHub - however, you can also use the PNG version by changing the file ending `.svg` to `.png`. Note that you can further customize the button's behavior through the use of [query string parameters](deploy-with-balena-button.md#query-string-parameters).
...
\[balenahub]:\{{ $links.balenaHubUrl \}} \[configuration]:/learn/manage/configuration \[variables]:/learn/manage/variables \[create-your-block]:/learn/develop/blocks/#creating-your-own-block
