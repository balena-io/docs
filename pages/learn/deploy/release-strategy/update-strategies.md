...
* `{{ $names.company.allCaps }}_SUPERVISOR_UPDATE_STRATEGY` and
* `{{ $names.company.allCaps }}_SUPERVISOR_HANDOVER_TIMEOUT`.

Setting `{{ $names.company.allCaps }}_SUPERVISOR_UPDATE_STRATEGY` to a valid value selects the update strategy. The possible values are:
...
which are explained below. `{{ $names.company.allCaps }}_SUPERVISOR_HANDOVER_TIMEOUT` is only used in the  `hand-over` strategy, and its use is explained in the [strategy's description](#hand-over).
...
* If the file is not created after a time defined in `{{ $names.company.allCaps }}_SUPERVISOR_HANDOVER_TIMEOUT`, the Supervisor kills the old version.

The `{{ $names.company.allCaps }}_SUPERVISOR_HANDOVER_TIMEOUT` variable defines this timeout in milliseconds, and defaults to 60000 (i.e. 1 minute).
