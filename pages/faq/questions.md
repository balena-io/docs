...
* [Can I use multiple containers?](questions.md#can-i-use-multiple-containers)
* [Can I mix device types in a fleet?](questions.md#can-i-mix-device-types-in-a-fleet)
* [How do I push a new git repo to a fleet?](questions.md#how-do-i-push-a-new-git-repo-to-a-fleet)
* [Why does /data report weird usage?](questions.md#why-does-data-report-weird-usage)
* [What NTP servers do the devices use?](questions.md#what-ntp-servers-do-the-devices-use)
* [What network ports are required?](questions.md#what-network-ports-are-required)
* [Can I use balenaCloud in countries with restrictive firewalls such as China?](questions.md#can-i-use-balenacloud-in-countries-with-restrictive-firewalls-such-as-china)
* [Can I access /dev and things like GPIO from the container?](questions.md#can-i-access-dev-and-things-like-gpio-from-the-container)
* [Can I set a static IP address for my device?](questions.md#can-i-set-a-static-ip-address-for-my-device)
* [Why can't I SSH into or run code in older versions of the host OS?](questions.md#why-cant-i-ssh-into-or-run-code-in-older-versions-of-the-host-os)
* [Which data is persisted on devices across updates/power cycles?](questions.md#which-data-is-persisted-on-devices-across-updates-power-cycles)
* [Why does /data disappear when I move a device between fleets?](questions.md#why-does-data-disappear-when-i-move-a-device-between-fleets)
* [It appears that there is a centralized master running (in cloud) and agents running on devices. Is that accurate?](questions.md#it-appears-that-there-is-a-centralized-master-running-in-cloud-and-agents-running-on-devices.-is-tha)
* [What type of encryption do you use over OpenVPN? SSL/TLS/AES-256? Mutual key authentication? over SSH?](questions.md#what-type-of-encryption-do-you-use-over-openvpn-ssl-tls-aes-256-mutual-key-authentication-over-ssh)
* [What is the performance impact on the gateway device due to encryption?](questions.md#what-is-the-performance-impact-on-the-gateway-device-due-to-encryption)
* [How long does the update process run typically? Do you have any benchmark data? For now it appears to be quick for small updates.](questions.md#how-long-does-the-update-process-run-typically-do-you-have-any-benchmark-data-for-now-it-appears-to)
* [How does the device registration work over the VPN and how do you ensure the identity of the device on the first-time registration?](questions.md#how-does-the-device-registration-work-over-the-vpn-and-how-do-you-ensure-the-identity-of-the-device)
* [If the device is installed behind a proxy/firewall and can’t be reachable on internet via direct connection, what are the pitfalls?](questions.md#if-the-device-is-installed-behind-a-proxy-firewall-and-cant-be-reachable-on-internet-via-direct-conn)
* [How do you secure your own cloud to prevent malicious attack which may allow attacker to break-in to our systems?](questions.md#how-do-you-secure-your-own-cloud-to-prevent-malicious-attack-which-may-allow-attacker-to-break-in-to)
* [How long is a balenaOS release maintained for?](questions.md#how-long-is-a-balenaos-release-maintained-for)
* [When are device types discontinued?](questions.md#when-are-device-types-discontinued)
* [What does it mean when a device type is discontinued?](questions.md#what-does-it-mean-when-a-device-type-is-discontinued)
* [I have a device that is not on the supported devices list. Can it run on balena?](questions.md#i-have-a-device-that-is-not-on-the-supported-devices-list.-can-it-run-on-balena)
* [What to keep in mind when choosing power supply units?](questions.md#what-to-keep-in-mind-when-choosing-power-supply-units)
* [Does balena have access to my device, source code and images?](questions.md#does-balena-have-access-to-my-device-source-code-and-images)
* [What are generic architecture images?](questions.md#what-are-generic-architecture-images)
* [What is the difference between the Intel NUC and the Generic x86\_64 images?](questions.md#what-is-the-difference-between-the-intel-nuc-and-the-generic-x86_64-images)

#### **Can I use multiple containers?**

Multiple container fleets are supported, beginning with balenaOS v2.12.0. To run multiple containers, you will need to have a [microservices fleet](../../learn/manage/app-types/) and include a `docker-compose.yml` file at the root of your project. You can reference the [multicontainer documentation](../../learn/develop/multicontainer/) for more details on the supported configurations.
...
It is possible to have devices of [different types](../../reference/base-images/devicetypes/) in the same fleet, as long as they have the same or compatible architectures. For example, you could have a fleet with both Raspberry Pi 3 and BeagleBone Black devices, as they both use an ARMv7 processor. Or you could have both a Raspberry Pi 3 and a Raspberry Pi Zero, each provisioned with its device specific host OS image, but running the same ARMv6 container image. However, you could not have any Intel NUC devices as part of the same fleet, as those devices have x86-64 processors.

Regardless of type, all devices in your fleet will get the same container images. This means that if you have mixed device types you'll need to use an architecture-specific [base image](../../reference/base-images/balena-base-images/) in your [Dockerfile](../../learn/develop/dockerfile/), rather than one based on device type.
...
NTP servers used by devices are enumerated on the [time management](../../reference/OS/time/#networking-requirements) documentation page.

#### **What network ports are required?**

Please take a look at the [networking requirements](../../reference/OS/network/2.x#network-requirements) section of our documentation.
...
For fleets running [multiple containers](../../learn/develop/multicontainer/), you will either need to define services as privileged or use the `cap_add` and `devices` settings in the `docker-compose.yml` file to map in the correct hardware access to the container.

#### **Can I set a static IP address for my device?**

Yes! It's actually pretty easy. Have a look at the [network setup](../../reference/OS/network/2.x#setting-a-static-ip) section of our documentation. In general, most network configurations can be achieved by changing the NetworkManager configuration file.
...
Generally, we try to follow good OPSEC practices for our systems. We support 2FA for user accounts and force all the connections to be over HTTPS. More details on our approach can be found on our [security page](../../learn/welcome/security/).
